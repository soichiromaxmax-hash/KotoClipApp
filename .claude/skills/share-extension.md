...
### POST /api/words のリクエストボディ
```swift
func post(word: String, meaning: String, context: String,
          aiExampleNative: String, aiNotes: String, token: String) async throws {
    let aiExplanation = [aiExampleNative, aiNotes]
        .filter { !$0.isEmpty }
        .joined(separator: "\n")
    let body: [String: Any] = [
        "word": word,
...
`lookup` レスポンスの `meaning` / `aiExample`（→ context）/ `aiExampleNative` / `aiNotes` を使う。
