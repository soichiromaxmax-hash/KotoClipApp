...
- `C:/Users/SoichiroKamibeppu(MC/anki_app/` — バックエンド（FastAPI）+ Webフロント
- API本番: `https://kotoclip.onrender.com/api`

## デザインの唯一の正解
`anki_app/frontend/style.css` の `:root` CSS変数。iOSはウェブ仕様を完全再現する。
各画面の仕様基準: `anki_app/frontend/js/<画面名>.js`
...
## 現在のフェーズ（2026-05-06）
TestFlight でバグ修正を確認中。ビルドは EAS Build で随時反映中。
次ステップ: Share Extension 動作確認（App Group の Developer Portal 設定が必要）→ App Store 審査。
