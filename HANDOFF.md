...
**最終更新: 2026-07-12（Build #43リジェクト対応・匿名アカウント化・Build #45再提出・詳細は0-4章）**
...
## 0. 最新状況（2026-07-12 更新）★ここから読む

### 現在のビルド・OTA状態

| 項目 | 値 |
|---|---|
| 直近の EAS Build（iOS） | **Build #45**（2026-07-12実行・App Store Connectへアップロード済み） |
| App Store 審査 | Build #43が**2026-07-09にリジェクト**（0-4章参照）。指摘3件すべて対応し、Build #45を作り直して再アップロード済み。**ユーザー側でApp Store Connectの残作業（IAPスクリーンショット・説明文へのEULAリンク・プライバシーポリシーURL確認）を対応後、審査へ再提出する段階** |
| 次回 EAS Build 時のビルド番号 | **#46**（appVersionSource: remote による自動採番） |
| RevenueCat（課金機能） | ✅ 設定完了・Sandbox実購入検証済み。匿名アカウント購入→既存アカウント統合時の引き継ぎにも対応済み（0-4章） |
| OTA チャンネル | production チャンネル ✅ 稼働中。2026-07-12だけで10回程度OTA配信（0-4章の変更はすべてOTA反映済み） |
...
### 次にやること（優先順・2026-07-12更新）

| # | 内容 | 状態 |
|---|---|---|
| 1 | App Store Connect: IAP2商品にレビュー用スクリーンショット添付・審査提出 | ユーザー対応中（0-4章） |
| 2 | App Store Connect: 説明文に標準EULAリンク追記・プライバシーポリシーURL確認 | ユーザー対応中（0-4章） |
| 3 | 上記完了後、バージョン情報でBuild #45を選択し審査へ再提出 | ❌ 未実施 |
| 4 | iOS Share Extensionで保存元URLも記録（現状は説明文を実装に合わせて削除しただけ） | ❌ 未着手・バックログ（`.claude`メモリのfeature-backlog参照） |
| 5 | RevenueCat 設定 | ✅ 完了 |
| 6 | KotoClip Plus フェーズ1（安全な課金基盤） | ✅ 完了。Sandbox実購入テストも完了（0-3章参照） |
| 7 | 価格変更（月額¥450・半年¥2,400） | ✅ 完了 |
| 8 | Render無料プランのスリープ対策（cron-job.orgで10分おきping） | ✅ 設定済み |
| 9 | KotoClip Plus フェーズ2（苦手語モード制限・Koto成長Lv制限・週間レポート分岐・広告・バッジ等） | ❌ 未着手 |
| 10 | 言語別フィルタリング（スペイン語/英語を別々に管理） | ❌ 後回し（30〜45分の作業） |
| 11 | Expoパッケージ10個がSDK推奨バージョンよりわずかに古い（要調査） | ⚠️ 意図的に保留中（下記12章参照） |
| 12 | 今後の機能バックログ（他言語対応・デスクトップ取り込み・写真OCR読み込み・オフライン対応） | ❌ 未着手・優先度未設定 |
...
## 0-4. 2026-07-12: Build #43リジェクト対応・匿名アカウント化・Build #45再提出

### リジェクト内容（Submission ID: `0b237a27-3327-4996-b264-87fa553ccb33`）

Build #43（version 1.0）が2026-07-09にApple審査で3件の指摘を受けリジェクトされた。

| Guideline | 指摘内容 | 対応 |
|---|---|---|
| 2.1(b) | IAP商品（月額/半年）が審査未提出 | App Store Connect側の手作業（ユーザー対応中） |
| 3.1.2(c) | 購入フローにプライバシーポリシー・利用規約(EULA)への機能するリンクが無い | `app/paywall.tsx`にリンク追加（対応済み） |
| 5.1.1(v) | アカウント不要の機能（単語保存・復習・クイズ）までログイン必須にしている | 匿名アカウント方式で対応済み（下記） |

### 匿名アカウント方式（Guideline 5.1.1(v)対応）

**最終仕様**: 初回起動時、ログイン/新規登録/**「ログインなしで使用してみる」**の3択画面（`app/auth/login.tsx`）を表示。ゲスト利用を選ぶとバックエンドが自動で匿名ユーザーを発行し、単語保存・復習・クイズが即使える。設定画面またはログイン画面から任意でメール・パスワードを登録すると、それまでのデータを引き継いだまま本アカウント化できる。

**バックエンド新規エンドポイント（`anki_app/auth_router.py`）:**
- `POST /api/auth/anonymous` — 匿名ユーザー発行（`sign_in_anonymously()`）。**Supabaseダッシュボードで「Anonymous Sign-ins」を有効化しないと動かない**（デフォルトOFF。Authentication → Sign In/Providers）。slowapiでper-IP `10/hour`のレート制限あり
- `POST /api/auth/upgrade` — 匿名→本アカウント化（同一user_idにemail/password付与、既存データはそのまま引き継がれる）。**重要**: Supabaseは`admin.update_user_by_id()`で認証情報を変更すると既存セッションを即失効させるため、直後に新email/passwordで`sign_in_with_password()`し直し新トークンを返す実装になっている
- `POST /api/auth/merge` — 匿名（または任意アカウント）のデータを**別の既存アカウント**へ統合。ブラウザ拡張機能連携のために追加（下記参照）。`words`テーブルの`UNIQUE(user_id, word, target_lang, native_lang)`制約に対応し、統合先に同じ単語があれば統合元の重複行を削除
- `GET /api/auth/me` — `is_anonymous`/`email`を返す

**開発中に見つかった3つの追加バグとその修正（すべてOTA配信済み）:**
1. 当初「トークンが無い時だけ」自動匿名ログインしていたため、以前ログインしていた端末でトークンが失効すると強制ログイン画面に落ちてしまいGuideline 5.1.1(v)を満たせていなかった → `context/auth.tsx`の`setAuthExpiredHandler`でも同じ匿名ログイン処理を再利用するよう修正
2. ブラウザ拡張機能・PC用Webアプリ（`anki_app/chrome_extension/`・`anki_app/frontend/`）はメール+パスワードログインのみ対応で匿名アカウントを認識できないため、スマホで匿名のまま使い始めたユーザーがPC側で別に新規登録すると**繋がらない2つの別アカウントに分裂するリスク**があった → 上記の3択ログイン画面＋`/api/auth/merge`で解消
3. 匿名のまま課金した後に既存アカウントへ「ログイン」で切り替える(=merge)と、RevenueCatのapp_user_idが変わるため課金状態が新アカウントに反映されない問題があった → `context/auth.tsx`の`loginOrMerge()`でmerge成功後に`Purchases.logIn()`→`restorePurchases()`を呼び、Apple ID側の購入情報を新アカウントへ復元するよう修正

### Build #44 → Build #45

匿名アカウント関連の全修正をOTAで配信した後、審査タイミングの不確実性（OTA反映前の古い画面が審査で見える可能性）を避けるため、最新コードを含んだ**Build #45を作り直して再提出**した（Build #44は当初のBuild #43リジェクト対応直後の状態のまま）。

### 外部コードレビューで発覚した5件（2026-07-12・すべて修正・OTA配信済み）

| 重要度 | 内容 | ファイル |
|---|---|---|
| 高 | `upgradeAccount`/`mergeAccount`がAsyncStorageの`user_id`を更新しておらず、統合後に再起動するとRevenueCatが古い匿名IDでログインし直される恐れ | `lib/api.ts` |
| 中 | how-to.tsxのスマホ向けガイドが「保存元URLも保存」と説明しているが、iOS Share Extension（`ios/ShareExtension/ShareViewController.swift`）は実際にはURLを送っていない（`public.url`を意図的に除外）。PC用ブラウザ拡張は`location.href`を正しく送信済みで対象外。ガイド文を実態に合わせて修正（URL実送信の実装は今後の課題としてバックログへ） | `app/how-to.tsx` |
| 中 | AI利用上限(429)時のエラーメッセージが「翻訳に失敗しました」のみで課金導線として弱かった | `app/(tabs)/add.tsx`・`app/word/[id].tsx` |
| 低 | 設定画面の無料枠ゲージの黄色判定が固定値80（無料上限50では実質発動しない） | `app/(tabs)/settings.tsx` |
| 低 | 単語帳のお気に入り判定が素の`Boolean(w.is_favorite)`で、バックエンドが`"0"`文字列を返すと誤ってお気に入り扱いになるリスク | `app/(tabs)/words.tsx`（通知設定の`isSettingOn()`と同様の正規化関数を導入） |

### その他の機能追加（同日・App Store対応と並行して実施）

- フラッシュカード（`app/flashcard.tsx`）: 開始前に「英→日」「日→英」を選べる画面を追加。毎回新しく開くたびに選択を求め、同一訪問内で連続プレイする場合（「もう一度10枚」等）は再度聞かない仕様
...
│   ├── paywall.tsx           ← RevenueCat ペイウォール（月額+年額。匿名時のアカウント作成案内あり）
│   ├── upgrade.tsx           ← 匿名→本アカウント化フォーム（auth/配下だとリダイレクトに巻き込まれるため直下に配置）
│   ├── auth/login.tsx        ← ログイン/新規登録/ゲスト利用の3択画面
│   ├── word/[id].tsx
│   └── (tabs)/
